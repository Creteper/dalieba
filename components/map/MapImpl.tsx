...
function CustomPopup({
  ...
  return (
    <Popup>
      ...
      <div
        className="absolute z-[1000] transform -translate-x-1/2 -translate-y-full min-w-[200px]"
        style={{
          left: position.x,
          top: position.y - 8,
        }}
      ...
      >
        <div className="relative">
          ...
          <div
            className="absolute left-1/2 -translate-x-1/2 border-8 border-transparent border-t-background/80 backdrop-blur-sm"
            style={{ bottom: "-16px" }}
          />
        </div>
      </div>
...
function MarkerTitle({
  ...
  if (currentZoom > maxZoom || currentZoom < minZoom || !pixelPosition)
    return null;
...
function MapImpl({
  ...
  // 处理图标配置
  const getMarkerIcon = (
    markerIcon?: NonNullable<MapImplProps["markers"]>[number]["icon"]
  ) => {
  ...
  return (
    <div className={cn("h-full w-full relative z-40", className)}>
      ...
      {hoveredMarker && (
        <div
          className="absolute z-[1000] min-w-[200px] transform -translate-x-1/2 -translate-y-full pointer-events-none"
          style={{
